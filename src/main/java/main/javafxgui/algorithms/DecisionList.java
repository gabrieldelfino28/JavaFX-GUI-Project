 ...
 * Warning: Most of the Variables might be in portuguese, though the input text from the menus are in english.
 */
...
public class DecisionList {
    ...
    public void Ex_16() {
        ...
        liquidWage wage = new liquidWage(); //Abstract class with all the data to calculate a generic Liquid Wage.

        //Inputs
        wage.hourQ = Double.parseDouble(JOptionPane.showInputDialog("Inform the hours quantity"));
        wage.hourV = Double.parseDouble(JOptionPane.showInputDialog("Inform the hours value"));
        wage.discount = Double.parseDouble(JOptionPane.showInputDialog("Inform the discount percentage"));
        wage.desN = Double.parseDouble(JOptionPane.showInputDialog("Inform the descendants quantity"));

        //Calculation
        wage.desN = wage.desN * 100;
        wage.discount = (wage.discount / 100);
        wage.sB = (wage.hourQ * wage.hourV);

        // Final Wage = Brutal Wage - (Brutal Wage * Discount percentage) + Descendants value
        wage.sL = (wage.sB - (wage.sL * wage.desN) + wage.desN);

        JOptionPane.showMessageDialog(null, "The Final value of the Liquid Wage is: " + wage.sL);
